...
export class UserProfile {
    ...
    renderGlobalRank() {
        ...
        const allUsers = StorageManager.get("community_users") || [];
        ...
        const rankIndex = sortedUsers.findIndex(u => u.id === currentUser.id);
        const position = rankIndex !== -1 ? rankIndex + 1 : sortedUsers.length;

        const percentile = (position / sortedUsers.length) * 100;
        ...
        if (position === 1) {
            ...
            tierClass = "rank-legendary";
        } else if (percentile <= 1) {
            rankTier = "Top 1%";
            tierClass = "rank-elite";
            ...
            rankTier = `Rank #${position}`;
            tierClass = "rank-standard";
        }

        rankEl.textContent = rankTier;
        rankEl.className = `stat-value ${tierClass}`;
    }
